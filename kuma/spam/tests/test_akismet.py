...
from . import VERIFY_URL_RE, CHECK_URL_RE, SPAM_URL_RE, HAM_URL_RE
...
class AkismetClientTests(SimpleTestCase):
    ...
    def test_verify_invalid_key_wrong_response(self):
        ...
        self.assertFalse(client.ready)
        self.assertEqual(client.key, 'api-key')
        self.assertRaises(AkismetError, client.verify_key)
        self.assertEqual(len(responses.calls), 2)
        self.assertEqual(responses.calls[0].response.text, 'fail')
        self.assertEqual(responses.calls[1].response.text, 'fail')
    ...
    def test_check_comment_ham(self):
        ...
        self.assertIn('comment_content=yada+yada',
